...
# Preserve DOCKER_CMD_PREFIX across re-source (install-new.sh re-sources
# common.sh after pinning the version tag; resetting would wipe sudo prefix)
if [ -z "${DOCKER_CMD_PREFIX+x}" ]; then
  DOCKER_CMD_PREFIX=()
fi
...
disable_ipv6() {
  # Disable IPv6 system-wide to force all traffic over IPv4.
  # Prevents Docker/apt/curl issues on networks with broken IPv6 routing.
  local conf="/etc/sysctl.d/99-disable-ipv6.conf"
  if [ -f "$conf" ] && grep -q "disable_ipv6 = 1" "$conf" 2>/dev/null; then
    log_info "IPv6 already disabled (${conf} exists)"
    return
  fi
  log_info "Disabling IPv6 (IPv4 only mode)"
  sudo tee "$conf" > /dev/null <<'SYSCTL'
net.ipv6.conf.all.disable_ipv6 = 1
net.ipv6.conf.default.disable_ipv6 = 1
net.ipv6.conf.lo.disable_ipv6 = 1
SYSCTL
  sudo sysctl --system > /dev/null 2>&1
  log_success "IPv6 disabled — all traffic will use IPv4"
}
...
download_service_assets() {
  ...
  if ! download_raw_file "$deploy_dir/update.sh" "Deployment_Doc/update.sh"; then
    log_warn "Failed to download update.sh"
  else
    chmod +x "$deploy_dir/update.sh"
  fi
  if ! download_raw_file "$deploy_dir/lib/common.sh" "Deployment_Doc/lib/common.sh"; then
    log_warn "Failed to download lib/common.sh"
  fi
